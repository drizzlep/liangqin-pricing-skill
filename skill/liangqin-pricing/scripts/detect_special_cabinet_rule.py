...
IRREGULAR_CUTOUT_KEYWORDS = ("异形", "避让", "包管", "包柱", "管道避让", "拐角避让")
IRREGULAR_CUTOUT_CONTEXT_KEYWORDS = ("管道", "柱子", "梁位", "拐角")
CARD_SEAT_KEYWORDS = ("餐边柜卡座", "卡座书柜", "卡座柜", "卡座")
CARD_SEAT_CONTEXT_KEYWORDS = ("餐边柜", "书柜", "柜体", "柜子")
...
def _looks_like_irregular_cutout(text: str) -> bool:
    return _contains_any(text, IRREGULAR_CUTOUT_KEYWORDS) or _contains_any(text, IRREGULAR_CUTOUT_CONTEXT_KEYWORDS)


def _looks_like_card_seat_cabinet(text: str) -> bool:
    if "卡座" not in text:
        return False
    if _contains_any(text, ("轨道", "插座", "电源", "底座")) and not _contains_any(text, CARD_SEAT_CONTEXT_KEYWORDS):
        return False
    return _contains_any(text, CARD_SEAT_KEYWORDS) and _contains_any(text, CARD_SEAT_CONTEXT_KEYWORDS)
...
def detect_rule(text: str) -> dict[str, object]:
    ...
    if _contains_any(normalized, FRIDGE_CABINET_KEYWORDS):
        ...
        return payload
    if _looks_like_card_seat_cabinet(normalized):
        payload = {
            "special_rule": "card_seat_cabinet",
            "next_required_field": "depth",
            "next_question": "这个卡座柜体我先只确认一个关键尺寸：卡座进深大概多少？确认这个尺寸后，我再继续往下收报价路径。",
        }
        payload.update(
            build_response_metadata(
                route="special_cabinet_rule",
        ...
        return payload
    if _looks_like_irregular_cutout(normalized):
        payload = {
            "special_rule": "irregular_cutout",
            "next_required_field": "cutout_size",
            "next_question": "这项存在异形避让，我先只确认一个关键尺寸：避让部分的宽和高大概是多少？如果你更方便，也可以直接告诉我避让部分正面投影面积。",
        }
        payload.update(
            build_response_metadata(
                route="special_cabinet_rule",
