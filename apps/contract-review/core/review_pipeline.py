...
def _derive_actionable_priority(
    *,
    automation_state: str,
    review_analysis: dict[str, Any],
) -> dict[str, Any]:
    issue_codes = set(review_analysis.get("issue_codes") or [])
    review_card = review_analysis.get("review_card") or {}
    review_priority = str(review_card.get("priority") or "normal").strip()
    if automation_state == "ocr_or_vision_required":
        return {"actionable_priority": "blocked_by_ocr", "actionable_priority_score": 3}
    if "missing_required_field" in issue_codes:
        return {"actionable_priority": "need_user_input", "actionable_priority_score": 1}
    if review_priority in {"p0", "p1"}:
        return {"actionable_priority": "ready_now", "actionable_priority_score": 0}
    if issue_codes:
...
def _render_review_markdown(review_payload: dict[str, Any], replay_payload: dict[str, Any]) -> str:
    ...
    if review_card:
        lines.extend(["", "## 审单卡", ""])
        lines.append(f"- verdict: `{review_card.get('verdict', '')}`")
        lines.append(f"- priority: `{review_card.get('priority', '')}`")
        if review_card.get("contract_total"):
            lines.append(f"- contract_total: `{review_card['contract_total']}`")
        if review_card.get("pricing_total"):
            lines.append(f"- pricing_total: `{review_card['pricing_total']}`")
        if review_card.get("best_match_target"):
            lines.append(f"- best_match_target: `{review_card['best_match_target']}`")
        if review_card.get("issue_summary"):
            lines.append(f"- issue_summary: {review_card['issue_summary']}")
        if review_card.get("next_actions"):
            for action in review_card.get("next_actions") or []:
    ...
    if issues:
        lines.extend(["", "## 差异 Issue", ""])
        for issue in issues:
            lines.append(
                f"- `{issue.get('severity', '')}` `{issue.get('issue_code', '')}`: {issue.get('title', '')}"
            )
            if issue.get("delta_value"):
                lines.append(f"  delta: {issue['delta_value']}")
            if issue.get("recommended_check"):
                lines.append(f"  recommended_check: {issue['recommended_check']}")
    ...
    if template_profile:
        lines.extend(["", "## 模板画像", ""])
        lines.append(f"- template_id: `{template_profile.get('template_id', '')}`")
        lines.append(f"- template_fingerprint: `{template_profile.get('template_fingerprint', '')}`")
        lines.append(f"- trust_score: `{template_profile.get('trust_score', '')}`")
...
def run_review_job(
    ...
    *,
    ...
    normalized_fields_payload = normalize_job_fields(job, template_runtime_root=job_dir.parents[1])
    ...
    template_profile = build_template_profile(
        ...
        runtime_root=job_dir.parents[1],
    )
    actionable_priority_payload = _derive_actionable_priority(
    ...
    review_payload["review_analysis"] = {
        ...
        "issue_codes": review_analysis["issue_codes"],
        "root_cause_breakdown": review_analysis["root_cause_breakdown"],
        "next_question": review_analysis["next_question"],
    }
    ...
    write_json(
        ...
        "automation_state": review_payload["automation_state"],
        "product_split_item_count": product_split_payload.get("item_count", 0),
        "actionable_priority": actionable_priority_payload["actionable_priority"],
        "actionable_priority_score": actionable_priority_payload["actionable_priority_score"],
    },
    )
    ...
    return {
        ...
        "issue_codes": list(review_analysis.get("issue_codes") or []),
        "issue_count": int(review_analysis.get("issue_count") or 0),
