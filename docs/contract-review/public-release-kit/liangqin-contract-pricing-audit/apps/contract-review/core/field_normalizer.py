...
from runtime_paths import resolve_pricing_scripts_dir
...
def normalize_job_fields(job: ReviewJob) -> dict[str, Any]:
    text_sources = _collect_text_sources(job.assets)
...
def _collect_text_sources(assets: list[SourceAsset]) -> list[dict[str, str]]:
...
def _precheck_quote_module():
    scripts_dir = resolve_pricing_scripts_dir()
